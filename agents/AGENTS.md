...
- **Navegación Táctil/Teclado**: Atajos de teclado (flechas, espacio) y zonas de clic.
  ...
  - **Zonas de Clic (Tap Zones) e Interacción**:
    - **Modo Simple y Doble**: Al hacer clic en los tercios laterales (izquierdo o derecho) de la imagen activa, se pasa de página de forma limpia sin alternar la visibilidad de los menús flotantes. Al hacer clic en el tercio central de la imagen o en el fondo canvas (fuera de la imagen), se alterna la visibilidad de los menús/overlays flotantes (barra superior y botones de control) sin cambiar de página.
    - **Modo Cascada**: Al hacer clic en cualquier parte de la pantalla (sea la imagen o el fondo canvas), se alterna la visibilidad de los menús/overlays flotantes sin provocar saltos de página ni paginación accidental.
    - **Cierre de Paneles/Cajones**: Al hacer clic fuera de los cajones abiertos de Configuración o Lista de Traducciones (es decir, en el lienzo o la imagen del lector), estos paneles se cierran automáticamente para facilitar el retorno rápido a la lectura.
- **Estado Persistente**: Configuración de layout, dirección, zoom, estética de overlays y paneles guardados en `localStorage`.
...
* **Visor y UI del Lector**:
  ...
  * **Personalización Estética de Overlays**: Controles integrados en el panel de configuración para ajustar la tipografía (Sans-serif, Serif, Monospace, Comic), escalar el tamaño del texto traducido, cambiar el color del texto y del fondo, y configurar la opacidad del globo de diálogo. Los ajustes cuentan con botones de reinicio rápido (reset) y se persisten localmente mediante `localStorage`. Adicionalmente, se implementaron optimizaciones para evitar el renderizado inline redundante, previniendo lags en la lectura continua.
  * **Interacción Inmersiva sin Distracciones**: Optimización de las zonas de clic en el visor para evitar saltos accidentales de página y despliegues intrusivos de menús durante la lectura. Se independizó el avance de página de la visibilidad del menú en modo simple/doble, se limitó la paginación en cascada a favor de alternar los controles, y se implementó el auto-cierre de cajones (Settings/Translations drawers) al pulsar fuera de ellos.

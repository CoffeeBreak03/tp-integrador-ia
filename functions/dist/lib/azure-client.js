...
class AzureClient {
    ...
    async readMockFile(filename) {
        try {
            ...
            const p = path_1.default.join(workspaceRoot, 'mocks', filename);
    ...
    async callTranslateModel(texts) {
        if (process.env.USE_MOCK_AZURE === 'true') {
            const mock = await this.readMockFile('translate-response.json');
            if (mock && Array.isArray(mock.translations)) {
                return JSON.stringify(mock.translations.map((t) => ({ original: t.original, translated: t.translated })));
            }
            return JSON.stringify([]);
        }

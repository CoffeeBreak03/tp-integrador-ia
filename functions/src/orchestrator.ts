...
export class PipelineOrchestrator {
   ...
   */
  private sortMangaBoxes(boxes: Array<{ id: number; y_min: number; x_min: number; y_max: number; x_max: number }>): Array<{ id: number; y_min: number; x_min: number; y_max: number; x_max: number }> {
   ...
   */
  private removeDuplicateOverlaps(translations: TranslationBox[]): TranslationBox[] {
    const toKeep = new Set<number>(translations.map(t => t.id));
